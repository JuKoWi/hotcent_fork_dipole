...
from hotcent.pos_op.slako_new import (
    INTEGRALS,
    dftbplus_to_full,
    UNIQUE_INTEGRALS,
...
class SlaterKosterIntegrator:
    ...
    def __init__(
        ...
        if format not in FILE_FORMAT_OPTIONS:
            raise ValueError(
                f"{format} is invalid file format option. Available formats: {FILE_FORMAT_OPTIONS}"
            )
        ...
        self.elem_pairs = list(
            itertools.product(no_repeats_types, repeat=2)
        )  # ordered pairs (e.g. Mo-S and S-Mo are different)
    ...
    def _read_sk_file_full(self, elem_pair, dipole):
        ...
        if dipole:
            ...
            for i, integral in enumerate(
                UNIQUE_INTEGRALS_POSOP
            ):  # verify that columns that should be equal are equal
                for equivalent in EQUIVALENT_INTEGRALS_POSOP[integral]:
                    ...
                    if not np.allclose(
                        sk_table_r_unique[:, i], sign * sk_table_r[:, idx]
                    ):
                        raise ValueError(
                            f"Equivalent columns in {myfile} are not identical"
                        )
            ...
            if homonuclear:
                if not (len(same_atom) == len(UNIQUE_ATOMIC_TRANSITIONS)):
                    raise ValueError(
                        f"Incorrect number of distinct atomic transition in {myfile}"
                    )
        ...
        if not (np.shape(data)[0] == n_points):
            raise ValueError(
                f"{elem_pair}.skf table block does not match the number of distance points in the header"
            )
        if not homonuclear:
            same_atom = None
    ...
    def _create_SH_file_dict(self, format):
        ...
        for element_comb in self.elem_pairs:
            if format == "DFTB+":
                delta_R, n_points, S, H, eigvals = self._read_sk_file_dftbplus(
                    elem_pair=element_comb
                )
            elif format == "full":
                delta_R, n_points, S, H, eigvals = self._read_sk_file_full(
                    elem_pair=element_comb, dipole=False
                )
            elif format == "unique":
                delta_R, n_points, S, H, eigvals = self._read_sk_file_unique(
                    elem_pair=element_comb, dipole=False
                )
            else:
                raise ValueError(
                    f"Reading routine for file format {format} not implemented"
                )
            S_sk_dict[element_comb] = SKTable(
                table_type="S",
                table=S,
                deltaR=delta_R,
                n_points=n_points,
                same_atom=[1, 1, 1],
            )  # assume the atomic functions to be orthonormal
            H_sk_dict[element_comb] = SKTable(
                table_type="H",
                table=H,
                deltaR=delta_R,
                n_points=n_points,
    ...
    def _create_dipole_file_dict(self, format):
        ...
        for element_comb in self.elem_pairs:
            if format == "DFTB+" or format == "unique":
                delta_R, n_points, r, atom_transitions = self._read_sk_file_unique(
                    elem_pair=element_comb, dipole=True
                )
            elif format == "full":
                delta_R, n_points, r, atom_transitions = self._read_sk_file_full(
                    elem_pair=element_comb, dipole=True
                )
            else:
                raise ValueError(
                    f"Reading routine for file format {format} not implemented for position operator"
                )
            r_sk_dict[element_comb] = SKTable(
                table_type="r",
                table=r,
                deltaR=delta_R,
                n_points=n_points,
                same_atom=atom_transitions,
            )
    ...
    def _integrals_atom_pair(
        self, sk_table, posA, posB, operator, lmaxA, lmaxB, sk_table_dipole=None
    ):
        """
        ...
        """
        if operator == "r":
            assert sk_table_dipole is not None
        ...
        if EINSUM:
            euler_theta, euler_phi, euler_gamma = self._set_euler_angles(
                vec1=posA, vec2=posB
            )
            ...
            if self.contraction_path is None:
                self.contraction_path = np.einsum_path(
                    "ab,bc,dc -> ad", D_single, M, D_single, optimize="optimal"
                )[0]
            integrals = np.einsum(
                "ab,bc,dc -> ad", D_single, M, D_single, optimize=self.contraction_path
            )[:dimA, :dimB]
            if operator == "r":
                idx_pstart = 1
                idx_pend = 3
                ...
                if self.contraction_path_posop is None:
                    self.contraction_path_posop = np.einsum_path(
                        "ai, bj, ck, ijk -> abc",
                        D_single,
                        D_r,
                        D_single,
                ...
                position_elements = np.einsum(
                    "ai, bj, ck, ijk -> abc",
                    D_single,
                    D_r,
                    D_single,
                    M_posop,
                    optimize=self.contraction_path_posop,
                )[:dimA, :, :dimB]
            ...
            if operator == "r":
                ...
                position_elements = matrix_elements_posop(
        ...
        if operator == "r":  # consider origin shift
            ...
            shifted_dipole = position_elements + np.einsum(
                "ab, c -> acb", integrals, posA
            )
    ...
    def _assemble_atom_block(self, types, posA, posB, max_lA, max_lB, operator):
        ...
        if operator == "S":
            ...
            if same_atom:
                ...
                block = self._integrals_atom_pair(
                    sk_table=sk_table,
                    posA=posA,
                    posB=posB,
                    lmaxA=max_lA,
            ...
            if same_atom:
                ...
                block = np.transpose(block, (1, 0, 2))[
                    [2, 0, 1]
                ]  # put components as first axis and reorder y,z,x -> x,y,z
                for c in range(
                    3
                ):  # origin shift with overlap matrix (identity for intraatomic)
                ...
                block = self._integrals_atom_pair(
                    ...
                    lmaxB=max_lB,
                    operator="r",
                    sk_table_dipole=sk_table_dipole,
                )
        elif operator == "p":
            sk_table = self.S_sk_tables[types]
    ...
    def _calculate_lattice_dict(self, operator):
        ...
        atoms = self.aseAtoms
        lattice_dict = {}
        pairA, pairB, R, d = neighbor_list(
            "ijSd", a=atoms, cutoff=self.cutoff_dict, self_interaction=True
        )
        ...
        for i, pair in enumerate(pairA):
            ...
            if operator in ("r", "p"):
                ...
                matrix = lattice_dict.setdefault(
                    R_triple, np.zeros((self.total_orbs, self.total_orbs))
                )
            idxA = pairA[i]
            idxB = pairB[i]
            ...
            block = self._assemble_atom_block(
                types=(typeA, typeB),
                posA=posA,
                posB=posB,
                max_lA=maxlA,
            ...
            if operator in ("r", "p"):
                matrix[
                    ...
                    start_cols : start_cols + n_cols,
                ] = block
            else:
                matrix[
                    start_rows : start_rows + n_rows, start_cols : start_cols + n_cols
                ] = block
            ...
            for point in lattice_dict_p.keys():
                ...
                for m in range(np.shape(p_array)[1]):
                    for n in range(np.shape(p_array)[2]):
                        ...
                        j = n + 1
                        print(
                            f"{i} {j}\t{xre[m, n]:.18e}\t{xim[m, n]:.18e}\t{yre[m, n]:.18e}\t{yim[m, n]:.18e}\t{zre[m, n]:.18e}\t{zim[m, n]:.18e}",
                            file=f,
                        )
