...
from hotcent.pos_op.assemble_integrals import SK_Integral
from hotcent.pos_op.integrals import first_center, second_center, theta2, theta1, phi
...
def compare_integrals(zeta1, use_existing_skf=False, dipole=True):
    if not use_existing_skf:
        ...
        if dipole:
            off2c = Offsite2cTablePosOp(atom, atom, timing=True)
            off2c.run(rmin, dr, N, 
                      zeta=zeta_dict, 
                    #   nr=200, ntheta=500
                      )
