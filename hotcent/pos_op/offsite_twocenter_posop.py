...
class Offsite2cTablePosOp(MultiAtomIntegrator):
    ...
    def run(
        ...
        """
        Calculates off-site two-center position operator elements
        ...
        for i, R in enumerate(Rgrid):
            ...
            if i == Nsub - 1 or Nsub // 10 == 0 or i % (Nsub // 10) == 0:
                print(
                    "R=%8.2f, %i grid points ..." % (R, len(grid)),
                    file=self.txt,
                    flush=True,
                )
            ...
            for p, (e1, e2) in enumerate(self.pairs):
                ...
                if len(grid) > 0:
                    R_operator = self.calculate(
                        selected, e1, e2, R, grid, area, zeta=zeta
                    )
                    for j, key in enumerate(sorted(selected, key=lambda x: x[0][0])):
                        ...
                        tables[(p, bas1, bas2)][i, idx] = R_operator[
                            key
                        ]  # determines the order of sk-columns listed in sk-file?
        ...
        if smoothen_tails:
            ...
            for key in self.tables:
                for i in range(NUMSK_POSOP):
                    self.tables[key][:, i] = tail_smoothening(
                        self.Rgrid, self.tables[key][:, i]
                    )

        for p, (e1, e2) in enumerate(self.pairs):
            if e1 == e2:
                selected = select_integrals(e1, e2)
                self.atom_transition_dipole = self.calculate_atom_transitions(
                    selected=selected, e1=e1, e2=e2
                )
    ...
    def calculate_atom_transitions(self, selected, e1, e2):
        ...
        for integral_num in UNIQUE_ATOMIC_TRANSITIONS:
            ...
            if match is not None:
                ...
                angle_integral = sym.integrate(
                    sym.integrate(Y1 * Yr * Y2 * sym.sin(theta1), (phi, 0, 2 * sym.pi)),
                    (theta1, 0, sym.pi),
                )
                radial_integral = trapezoid(y=Rnl1 * Rnl2 * r**2 * r, x=r, dx=dr)
                r_int = (
                    np.sqrt(4 * np.pi / 3) * radial_integral * angle_integral.evalf()
                )
    ...
    def calculate(self, selected, e1, e2, R, grid, area, zeta):
        ...
        if zeta is not None:
            for key in selected:
                ...
                Rnl1 = e1.Rnl(r1, nl1)
                Rnl1 = (
                    N1 * r1 ** zeta[nl1][1] * np.exp(-zeta[nl1][0] * r1**2)
                )  # overwrite with gaussian for testing
                Rnl2 = e2.Rnl(r2, nl2)
                Rnl2 = (
                    N2 * r2 ** zeta[nl2][1] * np.exp(-zeta[nl2][0] * r2**2)
                )  # overwrite with gaussian for testing
                ...
                for bas2, valence2 in enumerate(e2.basis_sets):
                    ...
                    with open(filename, "w") as f:
                        write_skf(
                            ...
                            table=table,
                            mass=mass,
                            has_atom_transition=has_atom_transition,
                            atom_transitions=atom_transitions,
                            format=format,
                        )
