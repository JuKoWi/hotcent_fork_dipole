"""phi2 integrals and functions related to matrix element selection"""

import numpy as np
import math

"""nonvanishing SlaKo integrals over phi for H or S elements named in the form
Y1*Y2
sk_label: tuple
   (idx, l, m, l, m) with idx:unique number of integral according to increasing quantum numbers 
...
INTEGRALS = {
    ...
    (27, 1, -1, 3, -1): lambda c1, c2, s1, s2: 0.1875
    * np.sqrt(14)
    * (5 * c2**2 - 1)
    * s1
    * s2,
    ...
    * s2,
    ...
    (91, 2, -1, 3, -1): lambda c1, c2, s1, s2: 0.1875
    * np.sqrt(70)
    * (5 * c2**2 - 1)
    * s1
    * s2
    ...
    * c1,
    ...
    (164, 3, -2, 2, -2): lambda c1, c2, s1, s2: 0.9375
    * np.sqrt(7)
    * s1**2
    * s2**2
    * c1,
    (170, 3, -2, 3, -2): lambda c1, c2, s1, s2: 6.5625 * s1**2 * s2**2 * c1 * c2,
    (177, 3, -1, 1, -1): lambda c1, c2, s1, s2: 0.1875
    * np.sqrt(14)
    * (5 * c1**2 - 1)
    * s1
    * s2,
    (181, 3, -1, 2, -1): lambda c1, c2, s1, s2: 0.1875
    * np.sqrt(70)
    * (5 * c1**2 - 1)
    * s1
    * s2
    * c2,
    (187, 3, -1, 3, -1): lambda c1, c2, s1, s2: 0.65625
    * (5 * c1**2 - 1)
    * (5 * c2**2 - 1)
    * s1
    * s2,
    (192, 3, 0, 0, 0): lambda c1, c2, s1, s2: 0.25 * np.sqrt(7) * (5 * c1**3 - 3 * c1),
    (194, 3, 0, 1, 0): lambda c1, c2, s1, s2: 0.25
    * np.sqrt(21)
    * (5 * c1**3 - 3 * c1)
    * c2,
    (198, 3, 0, 2, 0): lambda c1, c2, s1, s2: 0.125
    * np.sqrt(35)
    * (5 * c1**3 - 3 * c1)
    * (3 * c2**2 - 1),
    (204, 3, 0, 3, 0): lambda c1, c2, s1, s2: 0.875
    * (5 * c1**3 - 3 * c1)
    * (5 * c2**3 - 3 * c2),
    (211, 3, 1, 1, 1): lambda c1, c2, s1, s2: 0.1875
    * np.sqrt(14)
    * (5 * c1**2 - 1)
    * s1
    * s2,
    (215, 3, 1, 2, 1): lambda c1, c2, s1, s2: 0.1875
    * np.sqrt(70)
    * (5 * c1**2 - 1)
    * s1
    * s2
    * c2,
    (221, 3, 1, 3, 1): lambda c1, c2, s1, s2: 0.65625
    * (5 * c1**2 - 1)
    * (5 * c2**2 - 1)
    * s1
    * s2,
...
"""
derivatives of phi2-integrals w.r.t. cos(theta) and sin(theta), used for Hamiltonian
when creating .skf file
"""
INTEGRAL_DERIVATIVE = {
    ...
    (12, 0, 0, 3, 0): lambda c1, c2, s1, s2: [
        0,
        0.25 * np.sqrt(7) * (15 * c2**2 - 3),
        0,
        0,
    ],
    (17, 1, -1, 1, -1): lambda c1, c2, s1, s2: [0, 0, 0.75 * s2, 0.75 * s1],
    (21, 1, -1, 2, -1): lambda c1, c2, s1, s2: [
        0,
        0.75 * np.sqrt(5) * s1 * s2,
        0.75 * np.sqrt(5) * c2 * s2,
        0.75 * np.sqrt(5) * c2 * s1,
    ],
    (27, 1, -1, 3, -1): lambda c1, c2, s1, s2: [
        0,
        1.875 * np.sqrt(14) * c2 * s1 * s2,
        0.1875 * np.sqrt(14) * s2 * (5 * c2**2 - 1),
        0.1875 * np.sqrt(14) * s1 * (5 * c2**2 - 1),
    ],
    (32, 1, 0, 0, 0): lambda c1, c2, s1, s2: [0.5 * np.sqrt(3), 0, 0, 0],
    (34, 1, 0, 1, 0): lambda c1, c2, s1, s2: [1.5 * c2, 1.5 * c1, 0, 0],
    (38, 1, 0, 2, 0): lambda c1, c2, s1, s2: [
        0.25 * np.sqrt(15) * (3 * c2**2 - 1),
        1.5 * np.sqrt(15) * c1 * c2,
        0,
        0,
    ],
    (44, 1, 0, 3, 0): lambda c1, c2, s1, s2: [
        0.25 * np.sqrt(21) * (5 * c2**3 - 3 * c2),
        0.25 * np.sqrt(21) * c1 * (15 * c2**2 - 3),
        0,
        0,
    ],
    (51, 1, 1, 1, 1): lambda c1, c2, s1, s2: [0, 0, 0.75 * s2, 0.75 * s1],
    (55, 1, 1, 2, 1): lambda c1, c2, s1, s2: [
        0,
        0.75 * np.sqrt(5) * s1 * s2,
        0.75 * np.sqrt(5) * c2 * s2,
        0.75 * np.sqrt(5) * c2 * s1,
    ],
    (61, 1, 1, 3, 1): lambda c1, c2, s1, s2: [
        0,
        1.875 * np.sqrt(14) * c2 * s1 * s2,
        0.1875 * np.sqrt(14) * s2 * (5 * c2**2 - 1),
        0.1875 * np.sqrt(14) * s1 * (5 * c2**2 - 1),
    ],
    (68, 2, -2, 2, -2): lambda c1, c2, s1, s2: [
        0,
        0,
        1.875 * s1 * s2**2,
        1.875 * s1**2 * s2,
    ],
    (74, 2, -2, 3, -2): lambda c1, c2, s1, s2: [
        0,
        0.9375 * np.sqrt(7) * s1**2 * s2**2,
        1.875 * np.sqrt(7) * c2 * s1 * s2**2,
        1.875 * np.sqrt(7) * c2 * s1**2 * s2,
    ],
    (81, 2, -1, 1, -1): lambda c1, c2, s1, s2: [
        0.75 * np.sqrt(5) * s1 * s2,
        0,
        0.75 * np.sqrt(5) * c1 * s2,
        0.75 * np.sqrt(5) * c1 * s1,
    ],
    (85, 2, -1, 2, -1): lambda c1, c2, s1, s2: [
        3.75 * c2 * s1 * s2,
        3.75 * c1 * s1 * s2,
        3.75 * c1 * c2 * s2,
        3.75 * c1 * c2 * s1,
    ],
    (91, 2, -1, 3, -1): lambda c1, c2, s1, s2: [
        0.1875 * np.sqrt(70) * s1 * s2 * (5 * c2**2 - 1),
        1.875 * np.sqrt(70) * c1 * c2 * s1 * s2,
        0.1875 * np.sqrt(70) * c1 * s2 * (5 * c2**2 - 1),
        0.1875 * np.sqrt(70) * c1 * s1 * (5 * c2**2 - 1),
    ],
    (96, 2, 0, 0, 0): lambda c1, c2, s1, s2: [1.5 * np.sqrt(5) * c1, 0, 0, 0],
    (98, 2, 0, 1, 0): lambda c1, c2, s1, s2: [
        1.5 * np.sqrt(15) * c1 * c2,
        0.25 * np.sqrt(15) * (3 * c1**2 - 1),
        0,
        0,
    ],
    (102, 2, 0, 2, 0): lambda c1, c2, s1, s2: [
        3.75 * c1 * (3 * c2**2 - 1),
        3.75 * c2 * (3 * c1**2 - 1),
        0,
        0,
    ],
    (108, 2, 0, 3, 0): lambda c1, c2, s1, s2: [
        0.75 * np.sqrt(35) * c1 * (5 * c2**3 - 3 * c2),
        0.125 * np.sqrt(35) * (3 * c1**2 - 1) * (15 * c2**2 - 3),
        0,
        0,
    ],
    (115, 2, 1, 1, 1): lambda c1, c2, s1, s2: [
        0.75 * np.sqrt(5) * s1 * s2,
        0,
        0.75 * np.sqrt(5) * c1 * s2,
        0.75 * np.sqrt(5) * c1 * s1,
    ],
    (119, 2, 1, 2, 1): lambda c1, c2, s1, s2: [
        3.75 * c2 * s1 * s2,
        3.75 * c1 * s1 * s2,
        3.75 * c1 * c2 * s2,
        3.75 * c1 * c2 * s1,
    ],
    (125, 2, 1, 3, 1): lambda c1, c2, s1, s2: [
        0.1875 * np.sqrt(70) * s1 * s2 * (5 * c2**2 - 1),
        1.875 * np.sqrt(70) * c1 * c2 * s1 * s2,
        0.1875 * np.sqrt(70) * c1 * s2 * (5 * c2**2 - 1),
        0.1875 * np.sqrt(70) * c1 * s1 * (5 * c2**2 - 1),
    ],
    (136, 2, 2, 2, 2): lambda c1, c2, s1, s2: [
        0,
        0,
        1.875 * s1 * s2**2,
        1.875 * s1**2 * s2,
    ],
    (142, 2, 2, 3, 2): lambda c1, c2, s1, s2: [
        0,
        0.9375 * np.sqrt(7) * s1**2 * s2**2,
        1.875 * np.sqrt(7) * c2 * s1 * s2**2,
        1.875 * np.sqrt(7) * c2 * s1**2 * s2,
    ],
    (153, 3, -3, 3, -3): lambda c1, c2, s1, s2: [
        0,
        0,
        3.28125 * s1**2 * s2**3,
        3.28125 * s1**3 * s2**2,
    ],
    (164, 3, -2, 2, -2): lambda c1, c2, s1, s2: [
        0.9375 * np.sqrt(7) * s1**2 * s2**2,
        0,
        1.875 * np.sqrt(7) * c1 * s1 * s2**2,
        1.875 * np.sqrt(7) * c1 * s1**2 * s2,
    ],
    (170, 3, -2, 3, -2): lambda c1, c2, s1, s2: [
        6.5625 * c2 * s1**2 * s2**2,
        6.5625 * c1 * s1**2 * s2**2,
        13.125 * c1 * c2 * s1 * s2**2,
        13.125 * c1 * c2 * s1**2 * s2,
    ],
    (177, 3, -1, 1, -1): lambda c1, c2, s1, s2: [
        1.875 * np.sqrt(14) * c1 * s1 * s2,
        0,
        0.1875 * np.sqrt(14) * s2 * (5 * c1**2 - 1),
        0.1875 * np.sqrt(14) * s1 * (5 * c1**2 - 1),
    ],
    (181, 3, -1, 2, -1): lambda c1, c2, s1, s2: [
        1.875 * np.sqrt(70) * c1 * c2 * s1 * s2,
        0.1875 * np.sqrt(70) * s1 * s2 * (5 * c1**2 - 1),
        0.1875 * np.sqrt(70) * c2 * s2 * (5 * c1**2 - 1),
        0.1875 * np.sqrt(70) * c2 * s1 * (5 * c1**2 - 1),
    ],
    (187, 3, -1, 3, -1): lambda c1, c2, s1, s2: [
        6.5625 * c1 * s1 * s2 * (5 * c2**2 - 1),
        6.5625 * c2 * s1 * s2 * (5 * c1**2 - 1),
        0.65625 * s2 * (5 * c1**2 - 1) * (5 * c2**2 - 1),
        0.65625 * s1 * (5 * c1**2 - 1) * (5 * c2**2 - 1),
    ],
    (192, 3, 0, 0, 0): lambda c1, c2, s1, s2: [
        0.25 * np.sqrt(7) * (15 * c1**2 - 3),
        0,
        0,
        0,
    ],
    (194, 3, 0, 1, 0): lambda c1, c2, s1, s2: [
        0.25 * np.sqrt(21) * c2 * (15 * c1**2 - 3),
        0.25 * np.sqrt(21) * (5 * c1**3 - 3 * c1),
        0,
        0,
    ],
    (198, 3, 0, 2, 0): lambda c1, c2, s1, s2: [
        0.125 * np.sqrt(35) * (15 * c1**2 - 3) * (3 * c2**2 - 1),
        0.75 * np.sqrt(35) * c2 * (5 * c1**3 - 3 * c1),
        0,
        0,
    ],
    (204, 3, 0, 3, 0): lambda c1, c2, s1, s2: [
        0.875 * (15 * c1**2 - 3) * (5 * c2**3 - 3 * c2),
        0.875 * (5 * c1**3 - 3 * c1) * (15 * c2**2 - 3),
        0,
        0,
    ],
    (211, 3, 1, 1, 1): lambda c1, c2, s1, s2: [
        1.875 * np.sqrt(14) * c1 * s1 * s2,
        0,
        0.1875 * np.sqrt(14) * s2 * (5 * c1**2 - 1),
        0.1875 * np.sqrt(14) * s1 * (5 * c1**2 - 1),
    ],
    (215, 3, 1, 2, 1): lambda c1, c2, s1, s2: [
        1.875 * np.sqrt(70) * c1 * c2 * s1 * s2,
        0.1875 * np.sqrt(70) * s1 * s2 * (5 * c1**2 - 1),
        0.1875 * np.sqrt(70) * c2 * s2 * (5 * c1**2 - 1),
        0.1875 * np.sqrt(70) * c2 * s1 * (5 * c1**2 - 1),
    ],
    (221, 3, 1, 3, 1): lambda c1, c2, s1, s2: [
        6.5625 * c1 * s1 * s2 * (5 * c2**2 - 1),
        6.5625 * c2 * s1 * s2 * (5 * c1**2 - 1),
        0.65625 * s2 * (5 * c1**2 - 1) * (5 * c2**2 - 1),
        0.65625 * s1 * (5 * c1**2 - 1) * (5 * c2**2 - 1),
    ],
    (232, 3, 2, 2, 2): lambda c1, c2, s1, s2: [
        0.9375 * np.sqrt(7) * s1**2 * s2**2,
        0,
        1.875 * np.sqrt(7) * c1 * s1 * s2**2,
        1.875 * np.sqrt(7) * c1 * s1**2 * s2,
    ],
    (238, 3, 2, 3, 2): lambda c1, c2, s1, s2: [
        6.5625 * c2 * s1**2 * s2**2,
        6.5625 * c1 * s1**2 * s2**2,
        13.125 * c1 * c2 * s1 * s2**2,
        13.125 * c1 * c2 * s1**2 * s2,
    ],
    (255, 3, 3, 3, 3): lambda c1, c2, s1, s2: [
        0,
        0,
        3.28125 * s1**2 * s2**3,
        3.28125 * s1**3 * s2**2,
...
NUMSK = len(INTEGRALS)
ALL_NONZERO_PHI2 = sorted([i[0] for i in INTEGRALS.keys()])

DICT_IDENTICAL_DFTBPLUS = {
    102: [102],
    85: [85, 119],
    68: [68, 136],
    38: [38],
...
DFTBPLUS_SIMPLE = (
    DICT_IDENTICAL_DFTBPLUS.keys()
)  # order in which the integrals appear in the conventional .skf format
...
UNIQUE_INTEGRALS = sorted(EQUIVALENT_INTEGRALS.keys())


def full_to_unique(table_full):
    table_unique = np.zeros((np.shape(table_full)[0], len(UNIQUE_INTEGRALS)))
    for i, num in enumerate(UNIQUE_INTEGRALS):
        idx = ALL_NONZERO_PHI2.index(num)
        table_unique[:, i] = table_full[:, idx]
    return table_unique


def unique_to_full(table_unique):
    table_full = np.zeros((np.shape(table_unique)[0], len(ALL_NONZERO_PHI2)))
    count = 0
    for i, integral in enumerate(UNIQUE_INTEGRALS):
        for equivalent in EQUIVALENT_INTEGRALS[integral]:
            ...
            table_full[:, idx] = table_unique[:, i]
            count += 1
    assert count == len(ALL_NONZERO_PHI2)
    return table_full


def full_to_dftbplus(table):
    """
    Convert a Slater-Koster table to the conventional format of .skf files in DFTB+
...
def dftbplus_to_full(path1, path2, to_file=False):
    """convert dftb-syle file to long, partwise redundant format.
    The long format allows computation of all matrix elements M_ij with i from A and j from B
    just from the file A-B.skf(long).

    For heteroatomic systems both parameter files A-B.skf(short) and B-A.skf(short) are
    required even to calculate just elements of the type M_ij with i from A and j from B.
    Therefore this conversion function requires two input files in that case.

    For homoatomic systems the missing elements can be found by multiplication with the parity factor (-1)**(l_i + l_j)

    """
    table1 = parse_dftb_file(path1, homonuclear=(path1 == path2))
    table2 = parse_dftb_file(path2, homonuclear=(path1 == path2))
    tableH1 = table1[:, :10]
    tableS1 = table1[:, 10:]
    tableH2 = table2[:, :10]
    tableS2 = table2[:, 10:]
    table_longH = np.zeros((np.shape(tableH1)[0], NUMSK))
    table_longS = np.zeros((np.shape(tableS1)[0], NUMSK))
    all_integrals = sorted([i[0] for i in INTEGRALS.keys()])
    # find integrals that are exactly identical and fill them into the table
    # dict_identical generated by integrals.identical_phi2()
    for i, idx_short in enumerate(DFTBPLUS_SIMPLE):
        for j in DICT_IDENTICAL_DFTBPLUS[idx_short]:
            idx_long = all_integrals.index(j)
            table_longH[:, idx_long] = tableH1[:, i]
            table_longS[:, idx_long] = tableS1[:, i]
    # go through the permuted element combination and fill in the table according to M_ij = (-1)**(l_i + l_j) * M_ji
    # dict_interchange generated with integrals.interchange_related_phi2
    dict_interchange = {
        102: [],
        85: [],
        68: [],
    ...
    for i, idx_short in enumerate(DFTBPLUS_SIMPLE):
        for j in dict_interchange[idx_short]:
            idx_long = all_integrals.index(abs(j))
            table_longH[:, idx_long] = math.copysign(1, j) * tableH2[:, i]
            table_longS[:, idx_long] = math.copysign(1, j) * tableS2[:, i]
    ...
    title = path1.split(".")[0]
    if to_file:
        combined_table = np.concatenate((table_longH, table_longS), axis=1)
        np.savetxt(
            fname=f"{title}_long.skf", X=combined_table, header=header, comments=""
        )
    return table_longH, table_longS


ANGULAR_MOMENTUM_SYMBOLS = {0: "s", 1: "p", 2: "d", 3: "f"}

"""hotcent native labels for orbitals of certain ml combination"""
HOTCENT_LABELS = {
    (0, 0): "s",
    (1, -1): "py",
    (1, 0): "pz",
...
def phi2(c1, c2, s1, s2, sk_label):
    """Returns the angle-dependent part of the given two-center H/S integral,
...
def select_subshells(val1, val2, sk_label):
    ...
    nl2 = None
    for nl in val2:
        if nl[1] == ANGULAR_MOMENTUM_SYMBOLS[sk_label[3]]:
            nl2 = nl
...
def write_skf(
    ...
    if format == "DFTB+":
        ...
        assert (numint % len(UNIQUE_INTEGRALS)) == 0
    ...
    if has_diagonal_data or has_offdiagonal_data:
        if has_diagonal_data:
            ...
            fields = ["H_f", "H_d", "H_p", "H_s", "S_f", "S_d", "S_p", "S_s"]
            labels = {}
        ...
        for prefix, d in zip(prefixes, dicts):
            for l_symbol in ["s", "p", "d", "f"]:
                if l_symbol in d:
                    key = "%s_%s" % (prefix, l_symbol)
                    labels[key] = d[l_symbol]
    ...
    if format == "DFTB+":
        ...
        numtab = numint // len(UNIQUE_INTEGRALS)
    assert numtab > 0
...
def parse_dftb_file(path, homonuclear):
    """Read a .skf file in the conventional format and return only the raw table entries (no header)"""
    data = []
