...
class SK_Integral:
    ...
    def choose_relevant_matrix(self):
        ...
        mat = np.array(self.Wigner_D_symb(self.euler_theta, self.euler_phi, self.euler_gamma), dtype=complex)
        self.Wigner_D = np.where(mat < 1e-15, 0, mat) 
        idx_pstart = 1
        idx_pend = 3
        D1 = self.Wigner_D
        D2 = self.Wigner_D
        D_r = self.Wigner_D[idx_pstart:idx_pend+1, idx_pstart:idx_pend+1] # only take p for operator
