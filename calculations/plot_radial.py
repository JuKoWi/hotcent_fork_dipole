...
def plot_radial_parts(atoms: list, orbs: list, rmax_au=4, rmax_log=8):
    ...
    for i, atom in enumerate(atoms):
        for j, orb in enumerate(orbs[i]):
            ...
            axs[1].semilogy(
                x_angstrom_log, np.abs(R_log), label=f"{atom.symbol}, {orb}"
            )
...
def find_similar_zeta(zeta, atoms: list, orbs: list, rmax_log, rmax_au=12):
    ...
    for i in range(3):
        if i == 1:
            ...
            R = (
                N1 * x_bohr ** (i + 1) * np.exp(-zeta[i] * x_bohr**2)
            )  # overwrite with gaussian for testing
            R_log = (
                N1 * x_bohr_log ** (i + 1) * np.exp(-zeta[i] * x_bohr_log**2)
            )  # overwrite with gaussian for testing
            axs[0].plot(x_angstrom, R, label=rf"$l$ = {i}, $\zeta$ = {zeta[i]}")
            axs[1].plot(
                x_angstrom_log, np.abs(R_log), label=f"$l$ = {i}, $\zeta$ = {zeta[i]}"
            )
    axs[0].set_xlabel(r"r $[\mathrm{\AA}]$")
    axs[1].set_xlabel(r"r $[\mathrm{\AA}]$")
