...
def slowest_decay_from_file(filename, num_dipole, threshold, atol=1e-7):
    ...
    idx = np.where(
        mask.any(axis=1), table.shape[1] - 1 - mask[:, ::-1].argmax(axis=1), -1
...
def plot_decay_file(filename, num_dipole, threshold, startline):
    ...
    keys, idx, data = slowest_decay_from_file(
        filename=filename, num_dipole=num_dipole, threshold=threshold
    )
    data = np.array(data)
    ...
    for i, key in enumerate(keys):
        ...
        ax.semilogy(
            r_angst[startline:],
            data[i, startline:],
            label=rf"$\langle {READABLE_LABELS[orba]}|\hat{{r}}_{{{comp}}}|{READABLE_LABELS[orbb]}\rangle$",
        )
    ax.legend()
...
def plot_dipole_decay(offsite_obj, num_dipole, threshold):
    keys, idx, data = find_slowest_decay(
        offsite_obj=offsite_obj, num_dipole=num_dipole, threshold=threshold
    )
...
def plot_dipole_decay_selected(
    sk_file, homonuclear, labels, readable_labels, eigvals, startline
):
...
def plot_overlap_decay(offsite_obj, num_overlap, threshold):
    keys, idx, data = find_slowest_overlap_decay(
        offsite_obj=offsite_obj, num_dipole=num_overlap, threshold=threshold
    )
    r = offsite_obj.Rgrid
    r_angst = bohr_to_angstrom(r)
    fig, ax = plt.subplots(figsize=(20, 9))
    ...
    for i, key in enumerate(keys):
        ...
        ax.semilogy(
            r_angst, data[i], label=rf"{typeA}-{typeB}: $\langle {orba}|{orbb}\rangle$"
        )
    ax.legend()
    ax.set_xlim(left=0, right=6)
    ax.set_xlabel(r"r / $\mathrm{\AA}$")
