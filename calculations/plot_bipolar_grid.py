...
def plot_bipolar(nr, nt, Rz):
    ...
    for i in range(nr - 1):
        ...
        Th0 = np.piecewise(
            h / R[i], [np.abs(h / R[i]) > 1], [np.nan, lambda x: np.arccos(x)]
        )
        Th1 = np.piecewise(
            h / R[i + 1], [np.abs(h / R[i + 1]) > 1], [np.nan, lambda x: np.arccos(x)]
        )
        ...
        A_list0 = [
            ...
            lambda x: rr * -x - (rr * -x + 0.5 * R[i + 1] ** 2 * (Th1 - Th0)),
            0.0,
            lambda x: 0.5 * h**2 * -np.tan(x) - 0.5 * R[i] ** 2 * -x,
            -1,
            np.nan,
        ]
        ...
        A_list1 = [
            lambda x: rr * x,
            lambda x: rr * x,
            lambda x: rr * x - (rr * Th0 - 0.5 * h**2 * (np.tan(Th1) - np.tan(Th0))),
    ...
    ax.plot(
        bohr_to_angstrom(grid[:, 0]),
        bohr_to_angstrom(grid[:, 1]),
        "o",
        ms=4,
        color="red",
    )
